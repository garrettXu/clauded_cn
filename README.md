...
> 本项目是学习和研究用途的工具，用于帮助学习网站结构、静态站点生成、资源本地化、部署流程和自动化质量检查，也可以用于制作你拥有版权或已获得授权的学习型网站。不要复刻未授权站点，不要冒用他人品牌、内容或服务。

## 学习用途声明

这个项目的目标是帮助开发者学习完整网站静态化流程：

- 学习如何发现页面、维护抓取队列和增量更新状态。
- 学习如何下载并重写图片、CSS、JS、字体、视频等静态资源。
- 学习如何把动态渲染页面转换成可部署的静态页面。
- 学习如何生成本地预览、Nginx 配置、部署说明和质量报告。
- 学习如何制作合法授权范围内的学习网站、文档镜像或内部知识站。

使用前必须确认目标网站属于你本人、你的组织，或已经获得明确授权。对于第三方未授权网站，只能用于分析公开技术模式，不能发布复刻内容。
